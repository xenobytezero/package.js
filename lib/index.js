...
PackageController.prototype.isLoaded = function(plugin) {
    return this.loadedPlugins.some(p => p.meta.name === plugin.meta.name);
}
...
PackageController.prototype.autoload = function(config) {
	...
	for (i = 0; i < libPackageJSONFiles.length; i++) {
        ...
        if (this.isLoaded(plugin)) { 
            if (this.config.debug){
				console.log("plugin " + plugin.meta.name + " already loaded. Skipping...");
			}
            continue;
        }
		...
		if (this.config.identify.bind(plugin)()){
            ...
            if (config.initializer !== undefined) {
                const pluginFunc = config.initializer(Plugin);
                plugin.instance = new pluginFunc(config.packageConstructorSettings);
            } else {
